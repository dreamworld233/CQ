...
| 问题 | 答案 |
|------|------|
| 我在哪里？ | T1–T9 全绿；T10–T14 全部推进（表现层/确定性测试/讲稿/验收清单/3 关连打），代码侧完成待用户目视+playtest+录屏；本地提交到 d5cd586 |
| 我要去哪里？ | 等用户 Unity 编译 + 跑 61 测试 + 目视 T10 三关连打；再按反馈收口 T11 打磨 / T12 回归 / T13 录屏 / T14 验收 |
...
- 下一步 = **用户侧验证**：`CQ/创建战斗测试场景` → Play，三关连打（关 1 杂兵 / 关 2 领主 / 关 3 领主+爪牙），看站位/左行动条/意图/手牌。
- 剩余收口：T11 打磨微调（按目视反馈）、T12 全量 NUnit 全绿 + 现场改 JSON 抽查、T13 录屏、T14 未过项核对。
