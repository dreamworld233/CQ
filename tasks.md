...
- [x] T10 2.5D 表现（左侧行动条 / 前后排站位 / 意图图标 / 手牌）
  - 产出：`Runtime/PositioningView.cs`（前后排站位）、`Runtime/UnitView.cs`（世界单位色块 + 名字/血量/意图 TextMesh）、`BattleController.cs` 重写（世界生成 + 左侧常驻行动条 + 手牌/技能/意图 HUD）。
  - 验收：编辑器 `CQ/创建战斗测试场景` → Play，世界前后排站位可视、左侧行动条常驻、敌人意图标签、手牌可交互。（⚠ 待用户目视验证）
- [ ] T11 打磨 + 数值微调
  - 待修（用户实测）：①行动条只显上回合顺序，应在新回合开始显本回合当前顺序；②点我方角色会把目标设成自己，导致一回合多角色同动时单体伤害打到自己、敌人不掉血。根因+修法见 `findings.md` 待修 bug。
- [~] T12 完整回归（NUnit 全量 + 确定性 + 零硬编码抽查）
  - 产出一部：`DeterminismTests.cs`（逐回合同 seed 同态）；零硬编码抽查点写入 `交付/面试讲稿.md`。
  - 待补：Unity 全量 NUnit 跑绿确认 + 现场改 JSON 抽查。
- [~] T13 录屏 + 讲稿 + 架构图
  - 产出一部：`交付/面试讲稿.md`（一句话可讲性 + 30s 口播 + 架构三件套 + mermaid + 2-3 分钟分镜）。
  - 待补：实际录屏。
- [~] T14 缓冲 + 验收核对
  - 产出一部：`交付/验收清单.md`（6 条验收逐条状态+证据）；`BattleController` 串 3 关顺序连打 + 「下一关」按钮；零硬编码审计写入 `findings.md`。
  - 待补：目视 T10 / playtest 打穿 3 关 / 录屏收口。
